...
(
    ...
    [Parameter(HelpMessage = 'Ẩn quảng cáo khỏi màn hình chính')]
    ...
    [Parameter(HelpMessage = 'Thay đổi giới hạn xóa audio cache.')]
    [Alias('cl')]
    [int]$cache_limit,
    
    [Parameter(HelpMessage = 'Tự động gỡ cài đặt các phiên bản Spotify có trong máy.')]
    [switch]$confirm_uninstall_ms_spoti,
    
    [Parameter(HelpMessage = 'Cài đè phiên bản Spotify đã lỗi thời hoặc không được hỗ trợ bằng phiên bản được đề xuất.')]
    ...
    [Parameter(HelpMessage = 'Bật tính năng thử nghiệm của Spotify')]
    [switch]$exp_spotify,
    ...
    [Parameter(HelpMessage = 'Kích hoạt giao diện mới')]
    [switch]$new_theme,

    [Parameter(HelpMessage = 'Bật màu thanh bên phải để khớp với màu bìa)')]
    [switch]$rightsidebarcolor,
    
    [Parameter(HelpMessage = 'Bật giao diện lyrics cũ')]
    [switch]$old_lyrics,
    
    [Parameter(HelpMessage = 'Không tạo shortcut trên desktop')]
    [switch]$no_shortcut,

    [Parameter(HelpMessage = 'Bật màu static cho lyrics.')]
    [string]$lyrics_stat,

    [Parameter(HelpMessage = 'Goofy')]
    [string]$urlform_goofy = $null,

    [Parameter(HelpMessage = 'Goofy.')]
    [string]$idbox_goofy = $null,

    [Parameter(HelpMessage = 'Lỗi khi ghi lại log vi string')]
    [switch]$err_vi,
    
    [Parameter(HelpMessage = 'Chọn ngôn ngữ muốn sử dụng để cài đặt. Mặc định là ngôn ngữ hệ thống được phát hiện.')]
    [Alias('l')]
    [string]$Language
)
...
New-Item -Type Directory -Name "Spotifie_Temp-$(Get-Date -UFormat '%Y-%m-%d_%H-%M-%S')" | Convert-Path | Set-Location
...
function Helper($paramname) {
    ...
    switch ( $paramname ) {
        ...
        "ViTranslate" { 
            # Additional translation of some words for the Vietnamese language
            $n = "vi.json"
            $contents = $webjsonvi.psobject.properties.name
            $json = $webjsonvi
        }
        "PodcastAd" { 
            # Aodcast ad block
    ...
    $contents | ForEach-Object { 
        ...
        $checkVer = $fr -and $to; $translate = $paramname -eq "ViTranslate"
...
If ($test_spa) {
    ...
    $patched_by_spotifie = $reader.ReadToEnd()
    $reader.Close()

    If ($patched_by_spotifie -match 'spotify patched by Bach#1337') {
        $zip.Dispose()    
    ...
    # Remove all languages except En and Vi from xpui.spa
    if ($ru) {
    ...
    ($zip_xpui.Entries | Where-Object { $_.FullName -match "i18n" -and $_.FullName -inotmatch "(vi|en.json|longest)" }) | ForEach-Object { $_.Delete() }
...
# Delete all files except "en" and "vi"
if ($ru) {
    $patch_lang = "$spotifyDirectory\locales"
    Remove-Item $patch_lang -Exclude *en*, *vi* -Recurse
}
